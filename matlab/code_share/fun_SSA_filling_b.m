...
if numel(Mlist)*numel(Klist) > 1 % cross validation is required
    ...
    for  iK = 1:numel(K)
        if iK == 1
            f = waitbar(iK/numel(K),sprintf('wait for cross validation: %d%%',round(iK/numel(K)*100)));
        else
            waitbar(iK/numel(K),f,sprintf('wait for cross validation: %d%%',round(iK/numel(K)*100)));
        end
